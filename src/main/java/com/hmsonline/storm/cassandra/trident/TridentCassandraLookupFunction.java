...
public class TridentCassandraLookupFunction<K, V> implements Function {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(TridentCassandraLookupFunction.class);
	...
	public TridentCassandraLookupFunction(TridentTupleMapper<K, V> tupleMapper,
	...
	public void prepare(Map stormConf, TridentOperationContext context) {
		...
		LOG.error("Creating new Cassandra Client @ (" + this.cassandraHost
	...
	public void execute(TridentTuple input, TridentCollector collector) {
		...
		try {
			...
			List<Values> valuesToEmit = columnsMapper.mapToValues(rowKey,
					colMap, input);
			...
			LOG.warn("Could not emit for row [" + rowKey + "] from Cassandra."
					+ " :" + e.getMessage(), e);
		}
	}
}
